...
fbaa_t* point_allocator;

point_t* point_malloc() {
    return (point_t*)fbaa_malloc(point_allocator);
}

void point_free(point_t* point) {
    fbaa_free(point_allocator, (void*)point);
}
...
void fbaa_test() {
    ...
    printf("full blocks:\n");
    print_mem(point_allocator->full_blocks, MEM_SIZE);

    printf(
        "free space: %d / %d\n",
        fbaa_available(point_allocator),
        MEM_SIZE
    );
    ...
    fbaa_destroy(free, point_allocator);
}
